...
public class TestDataController {
    ...
    public ResponseEntity<?> checkStock(@PathVariable Long productId) {
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "status",
                product.getStockQuantity() < 0 ? "Warning : negative stockQuantity" : " Normal stockQuantity"));
    }
    ...
    @PostMapping("/reset-shared-product")
    public ResponseEntity<?> resetSharedProduct() {
        sharedProductId = null;
        return ResponseEntity.ok(Map.of("message", "Shared product reset"));
    }
